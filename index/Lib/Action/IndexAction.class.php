...
class IndexAction extends Action {
    ...
    public function usertotal(){
        $User = M('User');
        $result = $User -> alias('u') -> field('u.id,u.type,u.content,u.addtime,u.name,n.sum,t.count') -> join('nokia_num as n ON n.uid = u.id') -> order('n.sum DESC,u.id ASC') -> join('LEFT JOIN (SELECT count(uid) as count,uid FROM nokia_share_code GROUP BY uid) as t ON t.uid = u.id') -> select();
        $this -> assign('result', $result);
